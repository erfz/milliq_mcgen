{
  ...
  if (goodFlag) {
    ...
    if (psi) {
      loadHist("../CMS-13-TeV/theory/psiLowPt/merged_jpsi.root","r2");
      loadHist("merged_jpsi_run3.root","r3");
    } else {
      loadHist("../CMS-13-TeV/theory/psiLowPt/merged_psiprime.root","r2");
      loadHist("merged_psiprime_run3.root","r3");
    }
  ...
  // Set colors, style, titles, naximum
    ...
    // save to pdf
    if (psi) {
      c1->SaveAs("jpsi_direct_Run2_Run3_comparison.pdf");
    } else {
      c1->SaveAs("psiprime_direct_Run2_Run3_comparison.pdf");
    }
  }
}
