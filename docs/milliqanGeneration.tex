...
{\bf HOWEVER:} The $P_T$ spectrum used in the generation of the events
was within $|y|<1.2$.  The event generation assumed a flat rapidity
spectrum, which is a good assumption, but it also assumed $\eta = y$ which
is not a good  assumption at low $P_T$.  (Suffices to say that at
$P_T=0$ the polar angle $\theta=\pm \pi$).  If $\frac{dN}{dy} = C$ is
flat, then $\frac{dN}{d\eta}= C \frac{d\eta}{dy}$.  Since the milliqan
detector is at $\eta=0.11$, we now also rescale the bottonium spectra
by an addotional factor of $\frac{d\eta}{dy}$ evaluated at $\eta=0.11$.

The factor $\frac{d\eta}{dy}$ evaluated at $\eta=0.11$ depends on
the mass and the $p_T$.  This factor is shown in
Fig.~\ref{fig:detady} for $\Upsilon$(1S) and $J\psi$ as a function of $p_T$.
The total Run 2 to Run 3 scale factors are
displayed in Figure~\ref{fig:ups_run_2_run_3_sf}.

\begin{figure}[H]
  \begin{center}
    \includegraphics[width=0.65\linewidth]{../misc/deta-dy.pdf}
...
\begin{figure}[H]
  \begin{center}
    \includegraphics[width=0.70\linewidth]{../oniaDirect/upsilon/Run2_to_Run3_scale.pdf}
...
were profided where in a rapidity ($y$) instead of peudo-rapidity ($\eta$) range.  This then
will necessitate a correction for $\frac{d\eta}{dy}$, which will suppress the
effective cross-section at low $P_T$, as discussed in
Section~\ref{sec:upsilon3}.  Furthermore, the $P_T$ spectra were generated with two
different calculations, one appropriate for low $P_T$ and one appropriate for high $P_T$.  We
...
Before propagation, this spectrum is rescaled to $|\eta|<1$
through a multiplication by 1.13/2 where the factor of 1.13 was
derived in Run2 from a MC study\footnote{Details of this study have been lost.}.
